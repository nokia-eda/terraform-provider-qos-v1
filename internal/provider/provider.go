...
import (
	...
	"github.com/nokia/eda/apps/terraform-provider-qos/internal/eda/utils"
...
var _ provider.Provider = (*qosProvider)(nil)

func New(ver string) func() provider.Provider {
	return func() provider.Provider {
		return &qosProvider{version: ver}
	}
}

type qosProvider struct {
	version string
}
...
func (p *qosProvider) Schema(ctx context.Context, req provider.SchemaRequest, resp *provider.SchemaResponse) {
...
func (p *qosProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
...
func (p *qosProvider) Metadata(ctx context.Context, req provider.MetadataRequest, resp *provider.MetadataResponse) {
	resp.TypeName = "qos-v1"
	resp.Version = p.version
}

func (p *qosProvider) DataSources(ctx context.Context) []func() datasource.DataSource {
	return []func() datasource.DataSource{
		NewAppGroupDataSource,
		NewEgressPolicyDataSource,
		NewEgressPolicyListDataSource,
		NewForwardingClassDataSource,
		NewForwardingClassListDataSource,
		NewIngressPolicyDataSource,
		NewIngressPolicyListDataSource,
		NewPolicyAttachmentDataSource,
		NewPolicyAttachmentListDataSource,
		NewPolicyDeploymentDataSource,
		NewPolicyDeploymentListDataSource,
		NewQueueDataSource,
		NewQueueListDataSource,
		NewResourceListDataSource,
	}
}

func (p *qosProvider) Resources(ctx context.Context) []func() resource.Resource {
	return []func() resource.Resource{
		NewEgressPolicyResource,
		NewForwardingClassResource,
		NewIngressPolicyResource,
